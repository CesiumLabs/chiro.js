 ...
 * @ignore
 */
function resolveTracks(results, requestorID) {
    if (!results.results.length)
        return { type: "NO_RESULT", tracks: [], requestorID };
    return (results.identifier === "ytsearch" || results.identifier == "scsearch") ?
        { type: "SEARCH_RESULT", tracks: results.results, requestorID } :
        {
            ...
            tracks: results.results[0].tracks.map((track) => encapsulateTrackData(track, requestorID)),
            requestorID
        };
}
 ...
 * @private
 */
function encapsulateTrackData(Track, requestor) {
    return {
        url: Track.url,
        title: Track.title,
